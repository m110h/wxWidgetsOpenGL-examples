...
public:
    myVec4() { x = y = z = w = 0.0 ;}
    myVec4(double xd, double yd, double zd, double wd): x(xd), y(yd), z(zd), w(wd) {}
    myVec4(const myVec3& v3, double wd = 0.0): x(v3.x), y(v3.y), z(v3.z), w(wd) {}
...
void MyOrtho(double left, double right, double bottom, double top, double zNear, double zFar, double *mo);
